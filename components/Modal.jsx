...
const Modal = ({ closeModal, isOpenModal, modalLang }) => {
    ...
    return (
        <div ref={modalRef} onClick={close} className="modal-bg fixed inset-0 flex items-center justify-center z-50 backdrop-blur-sm bg-[#4b4d6350]">
            <form onSubmit={handleSubmit(onSubmit)} onClick={(e) => e.stopPropagation()} className="modal bg-[#7d5a50] text-white p-6 rounded shadow-lg w-96 mx-3">
                ...
                <div className="mb-4">
                    <label className="block text-sm font-medium">{modalLang.firstName}</label>
                    <input
                ...
                <div className="mb-4">
                    <label className="block text-sm font-medium">{modalLang.lastName}</label>
                    <input
                ...
                <div className="mb-4">
                    <label className="block text-sm font-medium">{modalLang.number}</label>
                    <ReactInputMask
                ...
                <button disabled={isSubmitSuccessful} className="button w-full">
                    {isSubmitSuccessful ? <div className='w-5 h-5 m-auto rounded-full border-y animate-spin border-white' /> : modalLang.button}
                </button>
            </form>
        </div>
    )
